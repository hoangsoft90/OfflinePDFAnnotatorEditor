...
Environment: Expo SDK 56 (May 2026, RN 0.85, New Architecture always on, expo-router default), OpenSpec CLI 1.8.0, Node 24.
...
Scaffold via `npx create-expo-app` default template (already includes expo-router, TS, tabs). Routes: `app/(tabs)/index.tsx` (Home), `app/(tabs)/settings.tsx`. Future viewer/organizer screens mount at `app/viewer/[docId].tsx` and `app/organizer/[docId].tsx` without touching the tab bar (spec: deep-linkable routes).
...
`SQLiteProvider` + `openDatabaseAsync`. Single `documents` table (schema in spec). Migration strategy: `PRAGMA user_version` + simple versioned migration runner. No ORM — small typed data-access module (`src/db/documents-repo.ts`) keeps the DB layer swappable.
...
Original PDF is NEVER copied into app storage by the viewer path (read via content URI). Workspace is created lazily on first open by the `ProjectManager` (change `annotation`), but the layout constants + helpers (`storage-paths.ts`) are defined here.
