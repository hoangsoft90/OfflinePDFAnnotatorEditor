/**
 * Storage cleanup controls (storage/layout + files/recent specs) — web variant.
 * Clearing history removes recent rows + cached thumbnails but NEVER deletes
 * project workspaces that hold unsaved changes.
 */
...
export function clearThumbnailCache(): void {
  const dir = new WebDirectory('cache', 'thumbnails');
...
export function cleanupForRemoval(docs: Document[]): void {
  for (const doc of docs) {
    ...
    if (doc.isImportedCopy) {
      ...
      const copy = new WebFile(storagePaths.importsDir, `${doc.id}.pdf`);
...
export function removeTempFile(uri: string): void {
  try {
    if (uri.startsWith('webfs://') || uri.startsWith('data:')) {
      const f = new WebFile(uri.replace(/^webfs:\/\//, ''));
